...
static void char_timer_callback(struct timer_list *list);
atomic_t v = ATOMIC_INIT(0);
static struct char_demo_driver char_drv;
DEFINE_TIMER(char_timer, char_timer_callback);

static void char_timer_callback(struct timer_list *list)
{
    atomic_inc(&v);
    char_drv.devices[0].sec = atomic_read(&v);
    mod_timer(&char_timer, jiffies_64 + msecs_to_jiffies(1000));
}
/* ============================================================
 * 3. 文件操作函数
 * ============================================================ */
...
static int char_demo_open(struct inode *inode, struct file *filp)
{
    ...
    char_timer.expires = jiffies_64 + msecs_to_jiffies(1000);
                              ...
                              loff_t *f_pos)
{
    ...
    if(copy_to_user(buf, &dev->sec, sizeof(dev->sec)))
    {
        return -EFAULT;
    }
    memmove(dev->buf,
