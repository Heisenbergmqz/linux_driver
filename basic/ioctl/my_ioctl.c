...
static long char_demo_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    struct char_demo_dev *dev = filp->private_data;
    switch(cmd)
    {
        case CLEAR:
            ...
            break;
        case SET_VALUE:
            if(copy_from_user(&dev->value, (void __user*)arg, sizeof(dev->value)))
            {
                return -EFAULT;
            }
            break;
        case GET_VALUE:
            if(copy_to_user((void __user*)arg, &dev->value, sizeof(dev->value)))
            {
                return -EFAULT;
            }
            break;
